...
constexpr std::array<std::uint8_t, 256> kEcDefaultInitialStatus = {
    ,
};
...
class Rc4Crypto {
public:
  Rc4Crypto(const std::vector<std::uint8_t> &key,
  ...
  void Apply(std::uint8_t *data, const size_t length) {
    for (size_t offset = 0; offset < length; ++offset) {
      ...
      const auto keyByte =
          m_state[(m_state[m_i] + m_state[m_j]) % m_state.size()];
      if (data != nullptr) {
        data[offset] ^= keyByte;
      }
    }
  ...
  void Skip(const size_t length) { Apply(nullptr, length); }
  ...
  const std::vector<std::uint8_t> &State() const { return m_state; }
...
private:
  void EmitKey(const std::vector<std::uint8_t> &key) {
    ...
    for (size_t index1 = 0; index1 < m_state.size(); ++index1) {
      index2 = (index2 + m_state[index1] + key[index1 % key.size()]) %
               m_state.size();
...
bool ComputeMd5Bytes(const std::vector<std::uint8_t> &input,
  ...
  if (!input.empty() &&
      !CryptHashData(hash, input.data(), static_cast<DWORD>(input.size()), 0)) {
    if (outError != nullptr) {
  ...
  if (!CryptGetHashParam(hash, HP_HASHVAL, outHash.data(), &hashLength, 0) ||
      hashLength != outHash.size()) {
    if (outError != nullptr) {
...
BuildReversedMd5Text(const std::array<std::uint8_t, 16> &hash) {
...
bool BuildEncryptedSecretId(const std::string &password, const bool cryptEc,
                            std::vector<std::uint8_t> &outSecretId,
                            std::vector<std::uint8_t> &outIv,
                            std::string *outError) {
  const std::vector<std::uint8_t> passwordBytes(password.begin(),
                                                password.end());
  ...
  if (cryptEc) {
    ...
    Rc4Crypto ivBuilder(passwordBytes, std::vector<std::uint8_t>(
                                           kEcDefaultInitialStatus.begin(),
                                           kEcDefaultInitialStatus.end()));
...
bool ParseEncryptedSourceInfo(const std::vector<std::uint8_t> &inputBytes,
  ...
  if (outInfo.type == kMagicEncryptedSourceTypeEC) {
    ...
    const auto hintLength = static_cast<std::int32_t>(
        ReadLeU32(inputBytes.data() + sizeof(std::uint32_t) * 2));
    if (hintLength < 0) {
      if (outError != nullptr) {
    ...
    outInfo.overtLength =
        sizeof(std::uint32_t) * 3 + static_cast<size_t>(hintLength);
  ...
  if (outError != nullptr) {
    ...
    stream << "encrypted_source_type_unsupported: 0x" << std::hex
           << std::uppercase << outInfo.type;
    *outError = stream.str();
  }
  return false;
}

void ApplyEStdTransform(std::vector<std::uint8_t> &bytes,
  ...
  for (size_t offset = 0; offset < bytes.size(); offset += kCryptoBlockLength) {
    const size_t blockLength =
        (std::min)(kCryptoBlockLength, bytes.size() - offset);
    ...
    if (blockLength > remainedOvert) {
      ...
      blockCrypto.Apply(bytes.data() + offset + remainedOvert,
                        blockLength - remainedOvert);
      remainedOvert = 0;
    } else {
...
void ApplyEcTransform(std::vector<std::uint8_t> &bytes,
                      const size_t overtLength,
                      const std::vector<std::uint8_t> &secretId,
                      const std::vector<std::uint8_t> &iv) {
  Rc4Crypto keyTable({}, iv);
  ...
  for (size_t offset = 0; offset < bytes.size(); offset += kCryptoBlockLength) {
    ...
    Rc4Crypto blockCrypto(
        std::vector<std::uint8_t>(blockKey.begin(), blockKey.end()),
        std::vector<std::uint8_t>(kEcDefaultInitialStatus.begin(),
                                  kEcDefaultInitialStatus.end()));
    if (blockLength > remainedOvert) {
      blockCrypto.Skip(remainedOvert);
      blockCrypto.Apply(bytes.data() + offset + remainedOvert,
...
bool NormalizeEcDecryptedModuleBytes(std::vector<std::uint8_t> &bytes,
                                     std::string *outError) {
  ...
  if (ReadLeU32(bytes.data()) != kMagicFileHeader1 ||
      ReadLeU32(bytes.data() + sizeof(std::uint32_t)) != kMagicFileHeader2) {
    if (outError != nullptr) {
  ...
  while (offset < bytes.size()) {
    if (bytes.size() - offset <
        sizeof(RawSectionHeader) + sizeof(RawSectionInfo)) {
      if (outError != nullptr) {
    ...
    const size_t sectionLength = sizeof(RawSectionHeader) +
...
bool DecodeEncryptedSourceBytes(const std::vector<std::uint8_t> &inputBytes,
  ...
  if (!BuildEncryptedSecretId(readOptions.password, encryptedInfo.cryptEc,
                              secretId, iv, outError)) {
    return false;
  }
  ...
  if (decryptedBytes.size() <
      secretOffset + kEncryptedSecretIdLength + sizeof(std::uint32_t) * 2) {
    if (outError != nullptr) {
      *outError = "encrypted_source_body_too_small";
    }
    return false;
  }
  if (!std::equal(secretId.begin(), secretId.end(),
                  decryptedBytes.begin() +
                      static_cast<std::ptrdiff_t>(secretOffset))) {
    if (outError != nullptr) {
  ...
  outBytes.assign(
      decryptedBytes.begin() +
          static_cast<std::ptrdiff_t>(secretOffset + kEncryptedSecretIdLength),
      decryptedBytes.end());
  if (encryptedInfo.cryptEc &&
      !NormalizeEcDecryptedModuleBytes(outBytes, outError)) {
    return false;
  }
...
bool ReadModuleBytesWithReadOptions(const std::string &modulePath,
                                    const ReadOptions &readOptions,
                                    std::vector<std::uint8_t> &outBytes,
                                    std::string *outError) {
  ...
  return DecodeEncryptedSourceBytes(inputBytes, readOptions, outBytes,
                                    outError);
}
...
NormalizeModuleCommentLines(const std::string &rawComment,
                            const std::string &fallback) {
  std::vector<std::string> lines = SplitNormalizedLines(rawComment);
  lines.erase(std::remove_if(lines.begin(), lines.end(),
                             [](const std::string &item) {
                               return TrimAsciiCopy(item).empty();
                             }),
              lines.end());
  if (lines.empty()) {
    return {fallback};
...
std::string StripWrappedText(const std::string &text, const std::string &left,
                             const std::string &right) {
  if (!StartsWithText(text, left) || !EndsWithText(text, right) ||
      text.size() < left.size() + right.size()) {
    return text;
  }
...
bool NeedsEscapedTextLiteral(const std::string &text) {
  if (StartsWithText(text, kEscapedTextLiteralPrefix) ||
      StartsWithText(text, kEscapedLongTextLiteralPrefix)) {
    return true;
  }
...
std::string BuildDumpTextLiteral(const std::string &rawText,
                                 const bool isLongText) {
  if (!isLongText && !NeedsEscapedTextLiteral(rawText)) {
    return std::string(kTextLiteralLeftQuote) + rawText +
           kTextLiteralRightQuote;
  }

  const char *prefix =
      isLongText ? kEscapedLongTextLiteralPrefix : kEscapedTextLiteralPrefix;
  return std::string(kTextLiteralLeftQuote) + prefix +
         EscapeTextLiteralPayload(rawText) + kTextLiteralRightQuote;
}

bool NeedsEscapedBodyLine(const std::string &text) {
  const std::string maskedPrefix = std::string("' ") + kEscapedBodyLinePrefix;
  if (StartsWithText(text, kEscapedBodyLinePrefix) ||
      StartsWithText(text, maskedPrefix)) {
    return true;
  }
  for (const unsigned char ch : text) {
    if (ch == '\r' || ch == '\n' || (ch < 0x20 && ch != '\t')) {
...
void AppendRenderedBodyLine(std::vector<std::string> &outLines,
                            const int indent, const std::string &body) {
  ...
  if (StartsWithText(body, "' ")) {
    outLines.push_back(indentText + "' " + kEscapedBodyLinePrefix +
                       BuildDumpTextLiteral(body.substr(2), false));
    return;
  }

  outLines.push_back(indentText + kEscapedBodyLinePrefix +
                     BuildDumpTextLiteral(body, false));
}
...
std::string BuildDefinitionLine(const std::string &type,
...
bool IsTxt2EPlaceholderStruct(const DataTypeInfo &item) {
  return TrimAsciiCopy(item.comment) == "txt2e placeholder" &&
         item.members.empty();
}
...
public:
  ...
  bool ReadBStr(std::string &out, bool &isNull) {
    ...
    out.assign(reinterpret_cast<const char *>(m_bytes.data() + m_pos),
  ...
  bool ReadStandardText(std::string &out) {
    ...
    out.assign(reinterpret_cast<const char *>(m_bytes.data() + start),
               m_pos - start);
    ++m_pos;
    return true;
  }
...
bool DecodeSectionName(const RawSectionInfo &info, std::string &outName) {
  ...
  if (!std::equal(std::begin(info.key), std::end(info.key),
                  kSectionNameNoKey.begin())) {
    size_t keyIndex = 1;
  ...
  outName.assign(reinterpret_cast<const char *>(nameBytes.data()),
                 nameBytes.size());
  outName = TrimAsciiCopy(RemoveTrailingNulls(std::move(outName)));
  return !outName.empty();
}

bool ReadBlockHeaders(ByteReader &reader,
                      std::vector<BlockHeader> &outHeaders) {
  outHeaders.clear();
  std::int32_t length = 0;
...
bool ParseVariableList(std::int32_t count,
  ...
  for (auto &item : outVars) {
    ...
    if (!itemReader.ReadI32(item.length) ||
        !itemReader.ReadI32(item.dataType) || !itemReader.ReadI16(item.attr) ||
...
bool ParseProgramHeader(ByteReader &reader, ProgramHeaderInfo &outHeader) {
  if (!reader.ReadI32(outHeader.versionFlag1) ||
      ...
      !reader.ReadI32(outHeader.flag1) || !reader.ReadI32(outHeader.flag2)) {
    return false;
  }
  ...
  return reader.ReadDynamicBytes(outHeader.icon) &&
         reader.ReadDynamicText(outHeader.debugCommandLine);
}

bool ParseCodePage(ByteReader &reader, CodePageInfo &outPage) {
  if (!reader.ReadI32(outPage.unk1) || !reader.ReadI32(outPage.baseClass) ||
  ...
  return reader.ReadI32(varCount) && reader.ReadDynamicBytes(varBytes) &&
...
bool ParseFunction(ByteReader &reader, FunctionInfo &outFunction) {
  ...
  if (!reader.ReadI32(localCount) || !reader.ReadDynamicBytes(localBytes) ||
      !ParseVariableList(localCount, localBytes, outFunction.locals) ||
      !reader.ReadI32(paramCount) || !reader.ReadDynamicBytes(paramBytes) ||
...
bool ParseDataType(ByteReader &reader, DataTypeInfo &outDataType) {
  ...
  return reader.ReadI32(memberCount) && reader.ReadDynamicBytes(memberBytes) &&
         ParseVariableList(memberCount, memberBytes, outDataType.members);
}

bool ParseDll(ByteReader &reader, DllInfo &outDll) {
  if (!reader.ReadI32(outDll.attr) || !reader.ReadI32(outDll.returnType) ||
  ...
  std::int32_t paramCount = 0;
  std::vector<std::uint8_t> paramBytes;
  return reader.ReadI32(paramCount) && reader.ReadDynamicBytes(paramBytes) &&
         ParseVariableList(paramCount, paramBytes, outDll.params);
}

bool ParseProgramSection(const std::vector<std::uint8_t> &bytes,
                         ProgramSection &outProgram) {
  ByteReader reader(bytes);
  ...
  if (!reader.ReadI32(globalCount) || !reader.ReadDynamicBytes(globalBytes) ||
...
bool ParseFormElements(ByteReader &reader,
                       std::vector<FormInfo::ElementInfo> &outElements) {
  outElements.clear();

  std::int32_t count = 0;
  std::int32_t size = 0;
  if (!reader.ReadI32(count) || !reader.ReadI32(size) || count < 0 ||
      size < 0) {
    return false;
  }
  ...
  const auto readInt32VectorWithLengthPrefix =
      [](ByteReader &itemReader, std::vector<std::int32_t> &outValues) -> bool {
    outValues.clear();
  ...
  const auto readByteVectorWithLengthPrefix =
      [](ByteReader &itemReader, std::vector<std::uint8_t> &outBytes) -> bool {
  ...
  for (std::int32_t i = 0; i < count; ++i) {
    ...
    const size_t itemEnd =
        itemPos + sizeof(std::int32_t) + static_cast<size_t>(itemLength);
    if (itemEnd > blockEnd) {
      return false;
    }
    ...
    if (!itemReader.ReadRaw(20, ignoredBytes) ||
        !itemReader.ReadStandardText(element.name)) {
      return false;
    }
    ...
    if (element.isMenu) {
      ...
      if (!itemReader.ReadStandardText(element.comment) ||
          ...
          !itemReader.ReadI32(eventCount) || eventCount < 0) {
        return false;
      }
      ...
      const size_t remaining = itemEnd >= itemReader.position()
                                   ? (itemEnd - itemReader.position())
                                   : 0;
...
bool ParseForm(ByteReader &reader, FormInfo &outForm) {
  return reader.ReadI32(outForm.unknown1) && reader.ReadI32(outForm.unknown2) &&
         reader.ReadDynamicText(outForm.name) &&
         reader.ReadDynamicText(outForm.comment) &&
         ParseFormElements(reader, outForm.elements);
}

bool ParseConstants(std::int32_t count, const std::vector<std::uint8_t> &bytes,
  ...
  for (auto &item : outConstants) {
    ...
    ByteReader itemReader(bytes);
    if (!itemReader.Skip(targetOffset) || !itemReader.ReadI32(item.length) ||
...
bool ParseResourceSection(const std::vector<std::uint8_t> &bytes,
                          ResourceSection &outResource) {
  ByteReader reader(bytes);
  std::vector<BlockHeader> headers;
  if (!ReadBlockHeaders(reader, headers)) {
    return false;
  ...
  return reader.ReadI32(constCount) && reader.ReadDynamicBytes(constBytes) &&
         ParseConstants(constCount, constBytes, outResource.constants) &&
         reader.ReadI32(outResource.reserve);
}

bool ParseClassPublicitySection(const std::vector<std::uint8_t> &bytes,
                                std::vector<ClassPublicityInfo> &outItems) {
  outItems.clear();
  if ((bytes.size() % 8u) != 0u) {
    return false;
  }
...
bool ParseEventIndicesSection(const std::vector<std::uint8_t> &bytes,
                              std::vector<IndexedEventInfo> &outItems) {
  outItems.clear();
  ...
  while (!reader.eof()) {
    ...
    if (!reader.ReadI32(item.formId) || !reader.ReadI32(item.unitId) ||
        !reader.ReadI32(item.eventId) || !reader.ReadI32(item.methodId)) {
      return false;
    }
    outItems.push_back(item);
  }
...
bool ParseFolderSection(const std::vector<std::uint8_t> &bytes,
                        FolderSectionInfo &outFolders) {
  outFolders = {};
  ByteReader reader(bytes);
  ...
  while (!reader.eof()) {
    ...
    if (!reader.ReadI32(expandValue) || !reader.ReadI32(folder.key) ||
        !reader.ReadI32(folder.parentKey) ||
        !reader.ReadDynamicText(folder.name) ||
        !reader.ReadI32(childByteLength) || childByteLength < 0 ||
        (childByteLength % 4) != 0) {
      return false;
    }
...
void ApplyEventIndicesToForms(ModuleSections &sections) {
  ...
  for (const auto &item : sections.eventIndices) {
    auto formIt = std::find_if(sections.resources.forms.begin(),
                               sections.resources.forms.end(),
                               [&item](const FormInfo &form) {
                                 return form.header.dwId == item.formId;
                               });
    ...
    auto elementIt =
        std::find_if(formIt->elements.begin(), formIt->elements.end(),
                     [&item](const FormInfo::ElementInfo &element) {
                       return element.id == item.unitId;
                     });
    ...
    if (std::find(elementIt->events.begin(), elementIt->events.end(),
                  eventPair) == elementIt->events.end()) {
      elementIt->events.push_back(eventPair);
    }
  }
}

bool ParseUserInfoSection(const std::vector<std::uint8_t> &bytes,
                          UserInfoSection &outUser) {
  ByteReader reader(bytes);
  return reader.ReadDynamicText(outUser.programName) &&
         ...
         reader.ReadI32(outUser.version1) && reader.ReadI32(outUser.version2);
}

std::string ReadCStyleTextAt(const std::vector<std::uint8_t> &bytes,
                             const size_t offset) {
...
bool ParseRemovedDefinedItems(ByteReader &reader,
                              std::vector<RemovedDefinedItemInfo> &outItems) {
  outItems.clear();
...
bool ParseLosableSection(const std::vector<std::uint8_t> &bytes,
                         LosableSectionInfo &outLosable) {
  outLosable = {};
...
bool ParseModuleSectionsFromBytes(const std::vector<std::uint8_t> &bytes,
  ...
  if (!reader.ReadU32(header1) || !reader.ReadU32(header2) ||
      header1 != kMagicFileHeader1 || header2 != kMagicFileHeader2) {
    if (outError != nullptr) {
  ...
  while (!reader.eof()) {
    ...
    if (sectionName == "系统信息段") {
      ...
      std::memcpy(&outSections.systemInfo, sectionBytes.data(),
                  sizeof(RawSystemInfoSection));
      ...
      if (!ParseClassPublicitySection(sectionBytes,
                                      outSections.classPublicities)) {
        if (outError != nullptr) {
...
bool ParseModuleSections(const std::string &modulePath,
                         ModuleSections &outSections, std::string *outError) {
  std::vector<std::uint8_t> bytes;
...
bool CaptureNativeSectionSnapshotsInternal(
    const std::vector<std::uint8_t> &bytes,
    std::vector<NativeSectionSnapshot> &outSnapshots, std::string *outError) {
  outSnapshots.clear();
...
std::vector<std::string>
ExtractReadableNullStrings(const std::vector<std::uint8_t> &bytes) {
...
bool ParseEComDependencies(const std::vector<std::uint8_t> &bytes,
                           std::vector<EComDependencyRecord> &outRecords) {
  outRecords.clear();
  if (bytes.empty()) {
    return true;
  }
  ...
  ByteReader reader(bytes);
  const auto readInt32Array = [](ByteReader &input,
                                 std::vector<std::int32_t> &outValues) -> bool {
    outValues.clear();
  ...
  for (std::int32_t dependencyIndex = 0; dependencyIndex < dependencyCount;
       ++dependencyIndex) {
    EComDependencyRecord record;
    if (!reader.ReadI32(record.infoVersion) || record.infoVersion < 0 ||
        record.infoVersion > 2 || !reader.ReadI32(record.fileSize) ||
    ...
    if (!reader.ReadDynamicText(record.name) ||
        !reader.ReadDynamicText(record.path)) {
      return false;
    }

    std::vector<std::int32_t> starts;
    std::vector<std::int32_t> counts;
    if (!readInt32Array(reader, starts) || !readInt32Array(reader, counts) ||
        starts.size() != counts.size()) {
      return false;
    }
...
std::string GetBuiltinTypeName(std::int32_t typeValue) {
  ...
  case -1:
    return "";
  case static_cast<std::int32_t>(0x80000000u):
    return "通用型";
  case static_cast<std::int32_t>(0x80000101u):
    return "字节型";
  case static_cast<std::int32_t>(0x80000201u):
    return "短整数型";
  case static_cast<std::int32_t>(0x80000301u):
    return "整数型";
  case static_cast<std::int32_t>(0x80000401u):
    return "长整数型";
  case static_cast<std::int32_t>(0x80000501u):
    return "小数型";
  case static_cast<std::int32_t>(0x80000601u):
    return "双精度小数型";
  case static_cast<std::int32_t>(0x80000002u):
    return "逻辑型";
  case static_cast<std::int32_t>(0x80000003u):
    return "日期时间型";
  case static_cast<std::int32_t>(0x80000004u):
    return "文本型";
  case static_cast<std::int32_t>(0x80000005u):
    return "字节集";
  case static_cast<std::int32_t>(0x80000006u):
    return "子程序指针";
  case static_cast<std::int32_t>(0x80000008u):
    return "条件语句型";
  case 65537:
    return "窗口";
  case 65539:
...
inline std::int32_t GetType(std::int32_t id) { return id & kMaskType; }

inline bool IsLibDataType(std::int32_t id) {
  return (id & kMaskType) == 0 && id != 0;
}

inline void DecomposeLibDataTypeId(std::int32_t id, std::int16_t &outLib,
                                   std::int16_t &outType) {
...
constexpr std::array<const char *, FIXED_WIN_UNIT_PROPERTY_COUNT>
  ...
  if (isWinUnit) {
    if (dataType.m_nPropertyCount > 0 &&
      ...
      for (int propertyIndex = 0; propertyIndex < dataType.m_nPropertyCount;
           ++propertyIndex) {
        memberNames.emplace_back(ReadSupportLibraryName(
            dataType.m_pPropertyBegin[propertyIndex].m_szName));
      }
      return memberNames;
    }
...
BuildSupportTypeEventNames(const LIB_DATA_TYPE_INFO &dataType) {
  ...
  for (int eventIndex = 0; eventIndex < dataType.m_nEventCount; ++eventIndex) {
    eventNames.emplace_back(
        ReadSupportLibraryName(dataType.m_pEventBegin[eventIndex].m_szName));
  }
  return eventNames;
}

std::string ResolveCoreLibraryFallbackMemberName(std::int32_t typeId,
...
BuildSupportLibraryCandidatePaths(const std::string &sourcePath,
...
std::string BuildCoreSupportLibraryWarning(const CoreSupportLibraryIssue issue,
  ...
  case CoreSupportLibraryIssue::NotFound:
    stream << Utf8Literal(u8"未找到核心支持库 ") << displayName
           << Utf8Literal(u8"。它是所有易语言源码都必须引用的核心支持库。当前仍"
                          u8"会继续处理，但支持库命令名可能退化为 "
                          u8"_Lib0CmdXXX，AI 解读和部分回包场景会受影响。")
           << Utf8Literal(u8"请检查易语言安装目录及其上级 lib，或注册表 "
                          u8"E.Document\\Shell\\Open\\Command。");
    break;
  case CoreSupportLibraryIssue::LoadFailed:
    ...
    if (errorCode == ERROR_BAD_EXE_FORMAT) {
      stream << Utf8Literal(
          u8"这通常表示当前 e-packager 与支持库位数不匹配，例如 x64 的 "
          u8"e-packager 尝试加载 x86 的 krnln.fne。");
    } else {
      stream << Utf8Literal(
          u8"这通常表示文件损坏、依赖缺失，或当前进程无法直接加载该支持库。");
    }
    stream << Utf8Literal(
        u8"当前仍会继续处理，但支持库命令名可能退化为 _Lib0CmdXXX。");
    break;
  case CoreSupportLibraryIssue::SymbolReadFailed:
    ...
    stream << Utf8Literal(
        u8"，但无法读取 GetLibInfo "
        u8"导出的命令/类型符号。当前仍会继续处理，但支持库命令名可能退化为 "
        u8"_Lib0CmdXXX。");
    break;
  }
  return stream.str();
...
public:
  SymbolResolver(
      const ProgramSection &program, const ResourceSection &resources,
      const std::string &sourcePath,
      const std::vector<RemovedDefinedItemInfo> *removedDefinedItems = nullptr)
      : m_program(program), m_resources(resources), m_sourcePath(sourcePath),
  ...
  std::string ResolveType(std::int32_t typeValue) {
    ...
    for (const auto &dataType : m_program.dataTypes) {
      if (dataType.header.dwId == typeValue ||
          dataType.header.dwUnk == typeValue) {
  ...
  std::string ResolveUserName(std::int32_t id) const {
    ...
    case epl_system_id::kTypeMethod:
      return "_Sub_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeGlobal:
      return "_Global_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeStaticClass:
      return "_Mod_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeDll:
      return "_Dll_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeClassMember:
      return "_Mem_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeFormControl:
      return "_Control_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeFormMenu:
      return "_Menu_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeConstant:
      return "_Const_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeFormClass:
      return "_FormCls_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeLocal:
      return "_Local_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeImageResource:
      return "_Img_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeSoundResource:
      return "_Sound_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeStructMember:
      return "_StructMem_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeStruct:
      return "_Struct_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeDllParameter:
      return "_DllParam_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeClass:
      return "_Cls_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeForm:
      return "_Form_0x" + ToHexSuffix(id);
    case epl_system_id::kTypeFormSelf:
      return std::string();
    default:
      break;
    }
    ...
    std::ostringstream stream;
    stream << "_User_0x" << std::hex << std::uppercase
           << static_cast<std::uint32_t>(id);
    return stream.str();
  }

  bool TryGetMethodOwnerName(std::int32_t methodId,
                             std::string &outName) const {
    if (const auto it = m_methodOwnerNameCache.find(methodId);
        it != m_methodOwnerNameCache.end()) {
      outName = it->second;
      return !outName.empty();
    }
    outName.clear();
  ...
  std::string ResolveLibCmdName(std::int16_t libraryIndex,
    ...
    if (symbols == nullptr || commandId < 0 ||
        static_cast<size_t>(commandId) >= symbols->commandNames.size()) {
      std::ostringstream stream;
      stream << "_Lib" << libraryIndex << "Cmd" << commandId;
      return stream.str();
    }
    const std::string &name =
        symbols->commandNames[static_cast<size_t>(commandId)];
    return name.empty() ? std::string("_Lib") + std::to_string(libraryIndex) +
                              "Cmd" + std::to_string(commandId)
                        : name;
  }

  std::string ResolveLibConstantName(std::int16_t libraryIndex,
                                     std::int32_t constantId) {
    if (!EnsureSupportLibraryCacheByLibraryId(libraryIndex)) {
      std::ostringstream stream;
    ...
    const auto *symbols = FindSupportLibrarySymbolsByLibraryId(libraryIndex);
    if (symbols == nullptr || constantId < 0 ||
        static_cast<size_t>(constantId) >= symbols->constantNames.size()) {
      std::ostringstream stream;
      stream << "_Lib" << libraryIndex << "Const" << constantId;
      return stream.str();
    }
    const std::string &name =
        symbols->constantNames[static_cast<size_t>(constantId)];
    return name.empty() ? std::string("_Lib") + std::to_string(libraryIndex) +
                              "Const" + std::to_string(constantId)
                        : name;
  }

  std::string ResolveLibTypeName(std::int16_t libraryIndex,
                                 std::int32_t typeId) {
    if (!EnsureSupportLibraryCacheByLibraryId(libraryIndex)) {
      std::ostringstream stream;
    ...
    const auto *symbols = FindSupportLibrarySymbolsByLibraryId(libraryIndex);
    if (symbols == nullptr || typeId < 0 ||
        static_cast<size_t>(typeId) >= symbols->dataTypes.size()) {
      std::ostringstream stream;
      stream << "_Lib" << libraryIndex << "Type" << typeId;
      return stream.str();
    }
    ...
    const std::string &name =
        symbols->dataTypes[static_cast<size_t>(typeId)].name;
    return name.empty() ? std::string("_Lib") + std::to_string(libraryIndex) +
                              "Type" + std::to_string(typeId)
                        : name;
  }

  std::string ResolveLibTypeMemberName(std::int16_t libraryIndex,
                                       std::int32_t typeId,
                                       std::int32_t memberId) {
    if (libraryIndex == 0) {
      const std::string fallbackName =
          ResolveCoreLibraryFallbackMemberName(typeId, memberId);
    ...
    const auto &members =
        symbols->dataTypes[static_cast<size_t>(typeId)].memberNames;
    ...
    return name.empty()
               ? std::string("_Lib") + std::to_string(libraryIndex) + "Type" +
                     std::to_string(typeId) + "Mem" + std::to_string(memberId)
               : name;
  }

  std::string ResolveLibTypeEventName(std::int32_t typeValue,
                                      std::int32_t eventId) {
    if (!epl_system_id::IsLibDataType(typeValue)) {
    ...
    if (!EnsureSupportLibraryCacheByLibraryId(libraryIndex)) {
      return std::string("_Lib") + std::to_string(libraryIndex) + "Type" +
             std::to_string(typeId) + "Event" + std::to_string(eventId);
    }
    ...
    const auto &eventNames =
        symbols->dataTypes[static_cast<size_t>(typeId)].eventNames;
  ...
  bool IsTabControlDataType(std::int32_t typeValue) {
    ...
    const std::uint16_t supportIndex =
        static_cast<std::uint16_t>(rawValue >> 16);
    const std::uint16_t typeIndex =
        static_cast<std::uint16_t>(rawValue & 0xFFFFu);
...
private:
  ...
  static std::string ParseDebugSymbolName(const std::string &rawComment) {
    ...
    if (comment.size() < 7 || comment[0] != '_' || comment[1] != '-' ||
        comment[2] != '@' || (comment[3] != 'M' && comment[3] != 'S') ||
        comment[4] != '<' || comment.back() != '>') {
      return std::string();
    }

    std::string name = comment.substr(5, comment.size() - 6);
    if (name.empty() || !IsDebugSymbolIdentifierStart(
                            static_cast<unsigned char>(name.front()))) {
      return std::string();
    }
    for (size_t index = 1; index < name.size(); ++index) {
      if (!IsDebugSymbolIdentifierContinue(
  ...
  static std::string BuildSequentialPlaceholderName(const std::string &prefix,
                                                    int &counter) {
    ++counter;
    return prefix + std::to_string(counter);
  }

  std::string ResolveVisibleName(const std::string &rawName,
  ...
  void BuildUserNameCache() {
    ...
    for (const auto &function : m_program.functions) {
      std::string functionName = ResolveVisibleName(
          function.name, function.comment, "匿名子程序_", anonymousMethodCount);
      if (functionName.empty() &&
          function.header.dwId == m_program.header.flag2) {
        functionName = "_启动子程序";
      }
    ...
    for (const auto &item : m_resources.constants) {
      ...
      m_userNameCache[item.marker] = ResolveVisibleName(
          item.name, item.comment, prefix, anonymousConstantCount);
    }
    for (const auto &item : m_resources.forms) {
      m_userNameCache[item.header.dwId] = ResolveVisibleName(
          item.name, item.comment, "匿名窗口_", anonymousFormCount);
      for (const auto &element : item.elements) {
        std::int32_t elementId = element.id;
        if (epl_system_id::GetType(elementId) == 0) {
          elementId |= element.isMenu ? epl_system_id::kTypeFormMenu
    ...
    if (m_removedDefinedItems != nullptr) {
      for (const auto &item : *m_removedDefinedItems) {
        ...
        if (const auto it = m_userNameCache.find(item.id);
            it != m_userNameCache.end()) {
  ...
  SupportLibrarySymbols *
  FindSupportLibrarySymbolsByLibraryId(std::int16_t libraryIndex) {
    if (libraryIndex < 0) {
      return nullptr;
    }
    return FindSupportLibrarySymbolsByTypeIndex(
        static_cast<std::uint16_t>(libraryIndex + 1));
  }

  SupportLibrarySymbols *
  FindSupportLibrarySymbolsByTypeIndex(std::uint16_t supportIndex) {
    if (supportIndex == 0) {
      return nullptr;
    }
    if (const auto it = m_supportCache.find(supportIndex);
        it != m_supportCache.end()) {
      return &it->second;
    }
    return nullptr;
  }
  ...
  bool EnsureSupportLibraryCacheByLibraryId(std::int16_t libraryIndex) {
    ...
    return EnsureSupportLibraryCacheByTypeIndex(
        static_cast<std::uint16_t>(libraryIndex + 1));
  }
  ...
  bool EnsureSupportLibraryCacheByTypeIndex(std::uint16_t supportIndex) {
    ...
    if (const auto it = m_supportCache.find(supportIndex);
        it != m_supportCache.end()) {
      return !it->second.dataTypes.empty() ||
             !it->second.commandNames.empty() ||
             !it->second.constantNames.empty();
    }
    ...
    symbols.fileName = GetFirstSupportLibraryToken(
        m_program.header
            .supportLibraryInfo[static_cast<size_t>(supportIndex - 1)]);
    ...
    const auto candidates =
        BuildSupportLibraryCandidatePaths(m_sourcePath, symbols.fileName);
    ...
    if (module != nullptr) {
      const auto *getInfoProc = reinterpret_cast<PFN_GET_LIB_INFO>(
      ...
      if (getInfoProc != nullptr) {
        ...
        if (libInfo != nullptr &&
          ...
          if (libInfo->m_nCmdCount > 0 &&
            ...
            symbols.commandNames.reserve(
                static_cast<size_t>(libInfo->m_nCmdCount));
            for (int i = 0; i < libInfo->m_nCmdCount; ++i) {
              symbols.commandNames.emplace_back(
                  ReadSupportLibraryName(libInfo->m_pBeginCmdInfo[i].m_szName));
            }
          }
          if (libInfo->m_nLibConstCount > 0 &&
            ...
            symbols.constantNames.reserve(
                static_cast<size_t>(libInfo->m_nLibConstCount));
            for (int i = 0; i < libInfo->m_nLibConstCount; ++i) {
              symbols.constantNames.emplace_back(
                  ReadSupportLibraryName(libInfo->m_pLibConst[i].m_szName));
            }
          }
          if (libInfo->m_nDataTypeCount > 0 &&
            ...
            for (int i = 0; i < libInfo->m_nDataTypeCount; ++i) {
              ...
              typeSymbols.isTabControl =
                  (dataType.m_dwState & LDT_IS_TAB_UNIT) != 0;
      ...
      // Intentionally keep the module loaded for the rest of the process
      // lifetime. Some third-party .fne libraries corrupt the process heap
      // during DLL detach.
    }

    const bool loaded = !symbols.dataTypes.empty() ||
                        !symbols.commandNames.empty() ||
                        !symbols.constantNames.empty();
    if (!loaded && IsCoreSupportLibraryFileName(symbols.fileName)) {
      if (!candidateExists) {
        AddRuntimeWarning(BuildCoreSupportLibraryWarning(
            CoreSupportLibraryIssue::NotFound, symbols.fileName, std::string(),
            ERROR_SUCCESS));
      } else if (!moduleLoaded) {
        AddRuntimeWarning(BuildCoreSupportLibraryWarning(
            CoreSupportLibraryIssue::LoadFailed, symbols.fileName,
            lastCandidatePath, lastLoadError));
      } else {
        AddRuntimeWarning(BuildCoreSupportLibraryWarning(
            CoreSupportLibraryIssue::SymbolReadFailed, symbols.fileName,
  ...
  std::string ResolveSupportLibraryType(std::int32_t typeValue) {
    ...
    const std::string &name =
        symbols->dataTypes[static_cast<size_t>(typeIndex - 1)].name;
...
void SetAnonymousTypeAlias(
    std::unordered_map<std::int32_t, std::string> &aliases, std::int32_t typeId,
...
bool MatchAllMemberTypes(const std::vector<VariableInfo> &members,
                         std::int32_t expectedType, size_t expectedCount) {
  if (members.size() != expectedCount) {
    return false;
  }
  return std::all_of(members.begin(), members.end(),
                     [expectedType](const VariableInfo &item) {
                       return item.dataType == expectedType;
                     });
}
...
bool MatchWndClassExShape(const std::vector<VariableInfo> &members) {
  ...
  return members[9].dataType == static_cast<std::int32_t>(0x80000005u) &&
...
BuildLocalAnonymousTypeAliasMap(const ModuleSections &sections) {
  ...
  for (const auto &item : sections.program.dataTypes) {
    ...
    if (MatchAllMemberTypes(item.members,
                            static_cast<std::int32_t>(0x80000501u), 4)) {
...
std::vector<std::filesystem::path>
BuildModuleCandidatePaths(const std::string &sourcePath,
                          const std::string &modulePathText) {
  return BuildModuleFileLookupCandidates(Utf8PathToPath(sourcePath),
                                         modulePathText);
}

AnonymousTypeHints BuildAnonymousTypeHints(const ModuleSections &sections,
                                           const std::string &sourcePath) {
  ...
  for (const auto &record : ecomRecords) {
    ...
    for (const auto &candidate : candidates) {
      ...
      const auto dependencyAliases =
          BuildLocalAnonymousTypeAliasMap(dependencySections);
      for (const auto &item : dependencySections.program.dataTypes) {
        ...
        for (const auto &member : item.members) {
          ...
          if (const auto aliasIt = dependencyAliases.find(member.dataType);
              aliasIt != dependencyAliases.end()) {
            hints.importedMemberAliases[BuildHintKey(ownerName, memberName)] =
                aliasIt->second;
          }
        }
      }
      ...
      for (const auto &item : dependencySections.program.dlls) {
        ...
        for (const auto &param : item.params) {
          ...
          if (const auto aliasIt = dependencyAliases.find(param.dataType);
              aliasIt != dependencyAliases.end()) {
            hints.importedDllParamAliases[BuildHintKey(dllName, paramName)] =
                aliasIt->second;
          }
        }
      }
...
std::string ResolveMemberTypeWithHints(const VariableInfo &info,
  ...
  if (const auto it = hints.localTypeAliases.find(info.dataType);
      it != hints.localTypeAliases.end()) {
    return it->second;
  }
  if (const auto it =
          hints.importedMemberAliases.find(BuildHintKey(ownerName, info.name));
...
std::string ResolveDllParamTypeWithHints(const VariableInfo &info,
  ...
  if (const auto it =
          hints.importedDllParamAliases.find(BuildHintKey(dllName, info.name));
...
std::string BuildMethodParameterLine(const VariableInfo &info,
                                     SymbolResolver &resolver) {
...
std::string BuildLocalVariableLine(const VariableInfo &info,
                                   SymbolResolver &resolver) {
  return BuildDefinitionLine(
      "局部变量", {
                      TrimAsciiCopy(resolver.ResolveUserName(info.marker)),
...
std::string BuildClassVariableLine(const VariableInfo &info,
                                   SymbolResolver &resolver) {
  return BuildDefinitionLine(
      "程序集变量", {
                        TrimAsciiCopy(resolver.ResolveUserName(info.marker)),
...
std::string BuildGlobalVariableLine(const VariableInfo &info,
                                    SymbolResolver &resolver) {
  return BuildDefinitionLine(
      "全局变量", {
                      TrimAsciiCopy(resolver.ResolveUserName(info.marker)),
...
std::string BuildStructMemberLine(const VariableInfo &info,
...
std::string BuildDllParameterLine(const VariableInfo &info,
                                  const std::string &resolvedType,
                                  SymbolResolver &resolver) {
  std::vector<std::string> flags;
  if ((info.attr & kVarAttrByRef) != 0) {
...
bool IsDependencyDefinedId(const std::vector<EComDependencyRecord> &records,
  ...
  for (const auto &record : records) {
    for (const auto &range : record.definedIds) {
      ...
      if (range.count > 0 && idNum >= startNum &&
          idNum < startNum + range.count) {
        return true;
      }
    }
  }
...
bool IsStructHidden(const DataTypeInfo &item) { return (item.attr & 0x2) != 0; }

bool IsStructPublic(const DataTypeInfo &item) { return (item.attr & 0x1) != 0; }

bool IsDllHidden(const DllInfo &item) { return (item.attr & 0x4) != 0; }
...
bool IsRootProgramPage(const CodePageInfo &page) { return page.baseClass == 0; }
...
std::string ResolveProgramPageLogicalName(const ProgramSection &program,
  ...
  for (const auto &candidate : program.codePages) {
    if (IsRootProgramPage(candidate) != IsRootProgramPage(page) ||
        !IsAnonymousProgramPage(candidate)) {
      continue;
    }
    ++ordinal;
...
bool ShouldKeepPage(const ModuleSections &sections,
  ...
  if (IsClassHidden(sections, page.header.dwId) ||
      IsDependencyDefinedId(dependencyRecords, page.header.dwId)) {
    return false;
  }
  if (IsAnonymousProgramPage(page)) {
    ...
    return std::any_of(functions.begin(), functions.end(),
                       [](const FunctionInfo *item) {
                         return item != nullptr && !IsImportedFunction(*item);
                       });
  }
...
std::string BuildVersionText(const ModuleSections &sections) {
  if (sections.hasUserInfo) {
    return std::to_string(sections.userInfo.version1) + "." +
           std::to_string(sections.userInfo.version2);
  }
  if (sections.hasSystemInfo) {
    return std::to_string(sections.systemInfo.compileMajor) + "." +
           std::to_string(sections.systemInfo.compileMinor);
  }
  return "0.0";
}
...
void BuildDependencies(const ModuleSections &sections, Document &outDocument) {
  ...
  for (const auto &rawText : sections.program.header.supportLibraryInfo) {
    ...
    const std::string majorOrMinor =
        tokens.size() > 3 ? tokens[3] : std::string();
  ...
  TraceLine("BuildDependencies support_lib_done count=" +
            std::to_string(outDocument.dependencies.size()));

  std::vector<EComDependencyRecord> ecomRecords;
  TraceLine("BuildDependencies ecom_parse_begin bytes=" +
            std::to_string(sections.ecomSectionBytes.size()));
  if (ParseEComDependencies(sections.ecomSectionBytes, ecomRecords)) {
    TraceLine("BuildDependencies ecom_parse_ok count=" +
              std::to_string(ecomRecords.size()));
    for (const auto &record : ecomRecords) {
      Dependency item;
  ...
  TraceLine("BuildDependencies end count=" +
            std::to_string(outDocument.dependencies.size()));
}

std::vector<const FunctionInfo *>
CollectPageFunctions(const ProgramSection &program, const CodePageInfo &page) {
...
std::string
RenderExpr(const Expr &expr, SymbolResolver &resolver,
           int expectedLowestPrecedence = (std::numeric_limits<int>::max)());
bool ParseExpression(ByteReader &reader, std::unique_ptr<Expr> &outExpr,
                     std::string *outError, bool parseMember = true);
bool ParseStatementBlock(ByteReader &reader,
                         std::unique_ptr<StatementBlock> &outBlock,
                         std::string *outError, bool stopOnLoopTailCall);
void AppendStatementLines(const StatementBlock &block, SymbolResolver &resolver,
                          int indent, std::vector<std::string> &outLines);

bool TryGetOperatorInfo(std::int32_t methodId, OperatorInfo &outInfo) {
  switch (methodId) {
  case 15:
    outInfo = {"*", 2, OperatorInfo::Type::Multi};
    return true;
  case 16:
    outInfo = {"/", 2, OperatorInfo::Type::Multi};
    return true;
  case 17:
    outInfo = {"\\", 3, OperatorInfo::Type::Multi};
    return true;
  case 18:
    outInfo = {"%", 4, OperatorInfo::Type::Multi};
    return true;
  case 19:
    outInfo = {"+", 5, OperatorInfo::Type::Multi};
    return true;
  case 20:
    outInfo = {"-", 5, OperatorInfo::Type::Multi};
    return true;
  case 21:
    outInfo = {"-", 1, OperatorInfo::Type::Unary};
    return true;
  case 38:
    outInfo = {"＝", 6, OperatorInfo::Type::Binary};
    return true;
  case 39:
    outInfo = {"≠", 6, OperatorInfo::Type::Binary};
    return true;
  ...
  case 44:
    ...
    return true;
  case 45:
    outInfo = {"且", 7, OperatorInfo::Type::Multi};
    return true;
  case 46:
    outInfo = {"或", 8, OperatorInfo::Type::Multi};
    return true;
  case 52:
    outInfo = {"＝", 9, OperatorInfo::Type::Binary};
    return true;
  default:
    return false;
  }
...
std::string RenderExprList(const std::vector<std::unique_ptr<Expr>> &items,
                           SymbolResolver &resolver) {
  std::string out;
  for (size_t i = 0; i < items.size(); ++i) {
    ...
    out +=
        items[i] == nullptr ? std::string() : RenderExpr(*items[i], resolver);
  }
  return out;
}

std::string RenderExpr(const Expr &expr, SymbolResolver &resolver,
                       int expectedLowestPrecedence) {
  switch (expr.kind) {
  ...
  case ExprKind::Constant:
    return expr.shortValue1 == -2
               ...
               : "#" + resolver.ResolveLibConstantName(expr.shortValue1,
                                                       expr.intValue1);
  case ExprKind::EnumConstant:
    return "#" +
           resolver.ResolveLibTypeName(expr.shortValue1, expr.shortValue2) +
           "." +
           resolver.ResolveLibTypeMemberName(expr.shortValue1, expr.shortValue2,
                                             expr.intValue1);
  ...
  case ExprKind::AccessArray:
    return (expr.target == nullptr ? std::string()
                                   : RenderExpr(*expr.target, resolver)) +
           "[" +
           (expr.extra == nullptr ? std::string()
                                  : RenderExpr(*expr.extra, resolver)) +
           "]";
  case ExprKind::AccessMember: {
    std::string prefix;
    if (expr.target != nullptr) {
      const bool omitTarget = expr.target->kind == ExprKind::Variable &&
                              epl_system_id::GetType(expr.target->intValue1) ==
    ...
    if (expr.shortValue1 == -2) {
      if (epl_system_id::GetType(expr.intValue2) == epl_system_id::kTypeForm &&
        ...
        return prefix +
               resolver.ResolveLibTypeMemberName(0, 0, expr.intValue1 - 1);
      }
      return prefix + resolver.ResolveUserName(expr.intValue1);
    }
    return prefix + resolver.ResolveLibTypeMemberName(
                        expr.shortValue1, expr.intValue2, expr.intValue1);
  }
  case ExprKind::Call: {
    if (expr.target == nullptr && expr.shortValue1 == 0) {
      OperatorInfo operatorInfo = {};
      if (TryGetOperatorInfo(expr.intValue1, operatorInfo)) {
        const bool needBracket =
            operatorInfo.precedence > expectedLowestPrecedence;
        std::string text;
        if (needBracket) {
          text += "(";
        }
        if (operatorInfo.type == OperatorInfo::Type::Unary) {
          ...
          if (!expr.items.empty() && expr.items[0] != nullptr) {
            text +=
                RenderExpr(*expr.items[0], resolver, operatorInfo.precedence);
          }
          ...
          if (expr.items.size() >= 2 && expr.items[1] != nullptr) {
            text += RenderExpr(*expr.items[1], resolver,
                               operatorInfo.precedence - 1);
            for (size_t i = 2; i < expr.items.size(); ++i) {
              text += " ";
              text += operatorInfo.text;
              text += " ";
              text += expr.items[i] == nullptr
                          ? std::string()
                          : RenderExpr(*expr.items[i], resolver,
                                       operatorInfo.precedence - 1);
            }
          }
        }
    ...
    if (expr.flagValue && expr.shortValue1 == -2) {
      ...
      if (resolver.TryGetMethodOwnerName(expr.intValue1, ownerName) &&
          !ownerName.empty()) {
        text += ownerName + ".";
      }
    }
...
bool ParseParamList(ByteReader &reader,
                    std::vector<std::unique_ptr<Expr>> &outItems,
                    std::string *outError) {
  outItems.clear();
  while (true) {
...
bool ParseCallExpressionWithoutType(ByteReader &reader,
                                    std::unique_ptr<Expr> &outExpr,
                                    std::string *outUnexaminedCode,
                                    std::string *outComment, bool *outMask,
                                    std::string *outError) {
  ...
  if (!reader.ReadI32(outExpr->intValue1) ||
      !reader.ReadI16(outExpr->shortValue1) || !reader.ReadI16(flag) ||
  ...
  if (!reader.eof()) {
    ...
    if (marker == 0x38) {
      ...
      if (outError != nullptr) {
        std::ostringstream stream;
        stream << "call_expression_marker_invalid@0x" << std::hex
               << std::uppercase
               << (reader.position() == 0 ? 0 : reader.position() - 1)
               << ":marker=0x" << std::setw(2) << std::setfill('0')
               << static_cast<unsigned int>(marker) << "|ctx="
               << BuildReaderHexContext(reader, reader.position() == 0
                                                    ? 0
                                                    : reader.position() - 1);
        *outError = stream.str();
      }
...
bool ParseExpression(ByteReader &reader, std::unique_ptr<Expr> &outExpr,
                     std::string *outError, bool parseMember) {
  outExpr.reset();
  ...
  case 0x1C:
    expr->kind = ExprKind::Constant;
    if (!reader.ReadI16(expr->shortValue1) ||
        !reader.ReadI16(expr->shortValue2)) {
      if (outError != nullptr) {
  ...
  case 0x21: {
    ...
    if (!ParseCallExpressionWithoutType(reader, callExpr, nullptr, nullptr,
                                        nullptr, outError) ||
        callExpr == nullptr) {
      return false;
    }
  ...
  default:
    if (outError != nullptr) {
      std::ostringstream stream;
      stream << "unknown_expression_type_" << std::hex << std::uppercase
             << static_cast<int>(type) << "@0x" << reader.position() - 1
  ...
  const bool allowMemberChain = expr->kind == ExprKind::Variable ||
  ...
  while (allowMemberChain && !reader.eof()) {
    ...
    if (nextType == 0x39) {
      ...
      if (!reader.ReadI32(wrapper->intValue1) ||
          !reader.ReadI32(wrapper->intValue2)) {
        if (outError != nullptr) {
      ...
      if (epl_system_id::IsLibDataType(wrapper->intValue2)) {
        std::int16_t libId = 0;
        std::int16_t typeId = 0;
        epl_system_id::DecomposeLibDataTypeId(wrapper->intValue2, libId,
                                              typeId);
...
bool ReadExpectedMarker(ByteReader &reader, std::uint8_t expected,
                        const char *errorCode, std::string *outError) {
  std::uint8_t marker = 0;
  if (!reader.ReadU8(marker) || marker != expected) {
    if (outError != nullptr) {
      ...
      stream << errorCode << "@0x" << std::hex << std::uppercase
             << (reader.position() == 0 ? 0 : reader.position() - 1)
             << ":expected=0x" << static_cast<int>(expected) << ",actual=";
...
bool ParseStatementBlock(ByteReader &reader,
  ...
  while (!reader.eof()) {
    ...
    } while (std::find(kKnownTypes.begin(), kKnownTypes.end(), type) ==
             kKnownTypes.end());

    if (type == 0x50 || type == 0x51 || type == 0x52 || type == 0x53 ||
        type == 0x54 || type == 0x6F) {
    ...
    if (type == 0x71 && stopOnLoopTailCall) {
      ...
      if (ParseCallExpressionWithoutType(reader, probeExpr, &ignoredCode,
                                         &ignoredComment, &ignoredMask,
                                         &probeError)) {
    ...
    if (type == 0x6D) {
      ...
      if (!ReadExpectedMarker(reader, 0x6E, "switch_first_case_missing",
                              outError)) {
        return false;
      }
      ...
      while (true) {
        ...
        if (!ParseCallExpressionWithoutType(
                reader, callExpr, &caseInfo.unexaminedCode, &caseInfo.comment,
                &caseInfo.mask, outError)) {
          return false;
        }
        ...
        if (!ReadExpectedMarker(reader, 0x53, "switch_case_end_missing",
                                outError)) {
          return false;
        }
      ...
      if (!ParseStatementBlock(reader, statement.defaultBlock, outError,
          ...
          !ReadExpectedMarker(reader, 0x74, "switch_tail_marker_missing",
                              outError)) {
        return false;
      }
    ...
    std::string comment;
    bool mask = false;
    if (!ParseCallExpressionWithoutType(reader, callExpr, &unexaminedCode,
                                        &comment, &mask, outError)) {
      return false;
    }
    ...
    if (type == 0x70) {
      ...
      if (!ParseStatementBlock(reader, loopBody, outError, true) ||
          !ReadExpectedMarker(reader, 0x71, "loop_end_call_missing",
                              outError)) {
        return false;
      }
      ...
      bool endMask = false;
      if (!ParseCallExpressionWithoutType(reader, endCallExpr,
                                          &endUnexaminedCode, &endComment,
                                          &endMask, outError)) {
        return false;
      }
    ...
    if (type == 0x6C) {
      ...
      if (!ParseStatementBlock(reader, statement.block, outError, false) ||
          ...
          !ReadExpectedMarker(reader, 0x73, "if_true_tail_marker_missing",
                              outError)) {
        return false;
      }
      outBlock->items.push_back(std::move(statement));
...
void AppendStatementLines(const StatementBlock &block, SymbolResolver &resolver,
  ...
  for (const auto &statement : block.items) {
    ...
    case StatementKind::IfTrue: {
      ...
      line += statement.unexaminedCode.empty()
                  ? ".如果真 (" +
                        (statement.condition == nullptr
                             ? std::string()
                             : RenderExpr(*statement.condition, resolver)) +
                        ")"
      ...
      AppendRenderedBodyLine(outLines, indent,
                             (statement.mask ? "' " : "") +
                                 std::string(".如果真结束"));
      break;
    }
    case StatementKind::IfElse: {
      ...
      if (statement.elseBlock != nullptr) {
        AppendStatementLines(*statement.elseBlock, resolver, indent + 1,
                             outLines);
      }
      AppendRenderedBodyLine(outLines, indent,
                             (statement.mask ? "' " : "") +
      ...
      line += statement.unexaminedCode.empty()
                  ? ".计次循环首 (" +
                        (statement.condition == nullptr
                             ...
                             : RenderExpr(*statement.condition, resolver)) +
                        ", " +
                        (statement.expr2 == nullptr
                             ? std::string()
                             : RenderExpr(*statement.expr2, resolver)) +
                        ")"
                  : "." + statement.unexaminedCode;
      if (!statement.comment.empty()) {
        line += "  ' " + statement.comment;
      ...
      line += statement.unexaminedCode.empty()
                  ? ".变量循环首 (" +
                        ...
                        (statement.expr2 == nullptr
                             ...
                             : RenderExpr(*statement.expr2, resolver)) +
                        ", " +
                        (statement.expr3 == nullptr
                             ? std::string()
                             : RenderExpr(*statement.expr3, resolver)) +
                        ", " +
                        (statement.expr4 == nullptr
                             ? std::string()
                             : RenderExpr(*statement.expr4, resolver)) +
                        ")"
                  : "." + statement.unexaminedCode;
      if (!statement.comment.empty()) {
        line += "  ' " + statement.comment;
    ...
    case StatementKind::SwitchBlock: {
      ...
      for (size_t i = 0; i < statement.switchCases.size(); ++i) {
        ...
        if (caseItem.unexaminedCode.empty()) {
          line += i == 0 ? ".判断开始 (" : ".判断 (";
          line += caseItem.condition == nullptr
                      ? std::string()
                      : RenderExpr(*caseItem.condition, resolver);
          line += ")";
        } else if (i == 0) {
          line += ".判断开始";
      ...
      AppendRenderedBodyLine(outLines, indent,
                             (statement.switchCases[0].mask ? "' " : "") +
                                 std::string(".默认"));
      if (statement.defaultBlock != nullptr) {
        AppendStatementLines(*statement.defaultBlock, resolver, indent + 1,
                             outLines);
      }
      AppendRenderedBodyLine(outLines, indent,
                             (statement.switchCases[0].mask ? "' " : "") +
...
bool TryRenderFunctionBody(const FunctionInfo &functionInfo,
                           SymbolResolver &resolver,
  ...
  if (!ParseStatementBlock(reader, block, &parseError, false) ||
      block == nullptr) {
    if (outError != nullptr) {
      *outError =
          parseError.empty() ? "function_body_parse_failed" : parseError;
    }
    return false;
  }
  if (reader.position() != functionInfo.expressionData.size()) {
    if (outError != nullptr) {
      ...
      stream << "function_body_trailing_bytes@0x" << std::hex << std::uppercase
             << reader.position();
      if (reader.position() < functionInfo.expressionData.size()) {
        stream << ":next=0x" << std::setw(2) << std::setfill('0')
               << static_cast<int>(
                      functionInfo.expressionData[reader.position()]);
      }
      *outError = stream.str();
    }
    return false;
...
void BuildProgramPages(const ModuleSections &sections,
                       const GenerateOptions &options, Document &outDocument) {
  SymbolResolver resolver(sections.program, sections.resources,
  ...
  for (const auto &pageInfo : sections.program.codePages) {
    ...
    if (!ShouldKeepPage(sections, dependencyRecords, pageInfo, functions,
                        options.includeImportedPages)) {
      continue;
    }
    const std::string logicalPageName =
        ResolveProgramPageLogicalName(sections.program, pageInfo);
    TraceLine("BuildProgramPages page=" + logicalPageName +
    ...
    AppendLine(page, BuildDefinitionLine(
                         "程序集",
                         {
                             page.name,
                             baseClassName,
                             IsProgramPagePublic(sections, pageInfo.header.dwId)
                                 ? "公开"
                                 : std::string(),
      ...
      try {
        AppendLine(
            ...
            BuildDefinitionLine(
                ...
                {
                    TrimAsciiCopy(
                        resolver.ResolveUserName(functionInfo->header.dwId)),
                    TrimAsciiCopy(
                        resolver.ResolveType(functionInfo->returnType)),
        ...
        if (TryRenderFunctionBody(*functionInfo, resolver, bodyLines,
                                  &bodyError)) {
          if (!bodyLines.empty()) {
            AppendLine(page, "");
          ...
          TraceLine("BuildProgramPages body_parse_failed func=" +
                    TrimAsciiCopy(functionInfo->name) + " error=" + bodyError);
          AppendLine(page, "");
          AppendLine(page, "' native_parse_failed: " + bodyError);
        }
      } catch (const std::exception &ex) {
        const std::string error = std::string("exception: ") + ex.what();
        TraceLine("BuildProgramPages exception func=" +
                  TrimAsciiCopy(functionInfo->name) + " error=" + error);
        AppendLine(page, "");
  ...
  TraceLine("BuildProgramPages end count=" +
            std::to_string(outDocument.pages.size()));
}
...
void BuildStructPage(const ModuleSections &sections,
                     const AnonymousTypeHints &hints, Document &outDocument) {
  ...
  AppendLine(page, "");
  for (const auto &item : sections.program.dataTypes) {
    if (IsTxt2EPlaceholderStruct(item) || IsStructHidden(item) ||
        IsDependencyDefinedId(dependencyRecords, item.header.dwId)) {
      continue;
    }
    std::string typeName = TrimAsciiCopy(item.name);
    if (typeName.empty()) {
      if (const auto it = hints.localTypeAliases.find(item.header.dwId);
          it != hints.localTypeAliases.end()) {
        typeName = it->second;
      } else {
...
void BuildDllPage(const ModuleSections &sections,
                  const AnonymousTypeHints &hints, Document &outDocument) {
  ...
  for (const auto &item : sections.program.dlls) {
    ...
    const std::string dllName =
        TrimAsciiCopy(resolver.ResolveUserName(item.header.dwId));
    AppendLine(
        page,
        BuildDefinitionLine(
            "DLL命令", {
...
void BuildConstantPage(const ModuleSections &sections, Document &outDocument) {
  ...
  const bool hasValueConstants = std::any_of(
      sections.resources.constants.begin(), sections.resources.constants.end(),
      [&](const ConstantInfo &item) {
        return item.pageType == kConstPageValue && !IsConstantHidden(item) &&
               !IsDependencyDefinedId(dependencyRecords, item.marker);
      });
  ...
  for (const auto &item : sections.resources.constants) {
    if (item.pageType != kConstPageValue || item.valueText == "<图片>" ||
        item.valueText == "<声音>" || IsConstantHidden(item) ||
        IsDependencyDefinedId(dependencyRecords, item.marker)) {
      continue;
    }
    std::string valueText = item.valueText;
    if (StartsWithText(valueText, kTextLiteralLeftQuote) &&
        EndsWithText(valueText, kTextLiteralRightQuote)) {
      valueText = BuildDumpTextLiteral(StripWrappedText(valueText,
                                                        kTextLiteralLeftQuote,
                                                        kTextLiteralRightQuote),
                                       item.longText);
    } else if (!valueText.empty() &&
               valueText.find_first_of("eE") != std::string::npos &&
               valueText.find_first_not_of("0123456789+-.eE") ==
                   std::string::npos) {
      try {
...
std::string BoolToEText(bool value) { return value ? "真" : "假"; }
...
std::string BuildFallbackFormElementXmlName(const FormInfo::ElementInfo &item) {
  if (epl_system_id::IsLibDataType(item.dataType)) {
    ...
    return "未知类型.Lib" + std::to_string(libId) + "." +
           std::to_string(typeId);
  }
  return "未知类型." + std::to_string(item.dataType);
}

std::string ResolveFormElementXmlName(const FormInfo::ElementInfo &item,
                                      SymbolResolver &resolver) {
...
const FormInfo::ElementInfo *FindFormSelfElement(const FormInfo &form) {
  for (const auto &item : form.elements) {
    if (!item.isMenu &&
        epl_system_id::GetType(item.id) == epl_system_id::kTypeFormSelf) {
      return &item;
    }
  }
  for (const auto &item : form.elements) {
...
void AppendXmlLine(FormXml &formXml, int indent, const std::string &text) {
  formXml.lines.push_back(std::string(static_cast<size_t>(indent) * 2, ' ') +
                          text);
}
...
BuildFormControlXmlAttributes(const FormInfo::ElementInfo &item,
...
std::vector<std::pair<std::string, std::string>>
BuildFormMenuXmlAttributes(const FormInfo::ElementInfo &item) {
  std::vector<std::pair<std::string, std::string>> attributes;
  attributes.emplace_back("名称", TrimAsciiCopy(item.name));
...
std::string BuildQualifiedMethodName(SymbolResolver &resolver,
  ...
  const std::string methodName =
      TrimAsciiCopy(resolver.ResolveUserName(methodId));
  if (resolver.TryGetMethodOwnerName(methodId, ownerName) &&
      !TrimAsciiCopy(ownerName).empty()) {
    return TrimAsciiCopy(ownerName) + "::" + methodName;
  }
  return methodName;
}

void AppendFormControlEventXmlLines(FormXml &formXml,
  ...
  for (const auto &[eventKey, handlerId] : item.events) {
    ...
    attributes.emplace_back(
        "名称", resolver.ResolveLibTypeEventName(item.dataType, eventKey));
    const std::string handlerName =
        BuildQualifiedMethodName(resolver, handlerId);
    if (!handlerName.empty()) {
      attributes.emplace_back("处理器", handlerName);
    }
    AppendXmlLine(formXml, indent,
                  BuildXmlOpenTag(tagName + ".事件", attributes, true));
  }
}

void AppendFormMenuEventXmlLines(FormXml &formXml,
                                 const FormInfo::ElementInfo &item,
                                 const int indent, SymbolResolver &resolver) {
  ...
  const std::string handlerName =
      BuildQualifiedMethodName(resolver, item.clickEvent);
  if (!handlerName.empty()) {
    attributes.emplace_back("处理器", handlerName);
  }
  AppendXmlLine(formXml, indent,
                BuildXmlOpenTag("菜单.事件", attributes, true));
}

void BuildFormXmlEntries(const ModuleSections &sections,
  ...
  for (const auto &form : sections.resources.forms) {
    ...
    auto rootAttributes =
        formSelf != nullptr
            ...
            : std::vector<std::pair<std::string, std::string>>();
    rootAttributes.insert(rootAttributes.begin(),
                          std::make_pair("名称", formXml.name));
    if (!TrimAsciiCopy(form.comment).empty()) {
      rootAttributes.insert(
          rootAttributes.begin() + 1,
          std::make_pair("备注", TrimAsciiCopy(form.comment)));
    }
    ...
    for (const auto &item : form.elements) {
      ...
      while (menuStack.size() >
             static_cast<size_t>((std::max)(item.level, 0))) {
        menuStack.pop_back();
      }
    ...
    if (!menuRoots.empty()) {
      ...
      std::function<void(size_t, int)> renderMenu = [&](size_t nodeIndex,
                                                        int indent) {
        ...
        if (node.children.empty() && !hasEventBinding) {
          AppendXmlLine(formXml, indent,
                        BuildXmlOpenTag("菜单", attributes, true));
          return;
        }

        AppendXmlLine(formXml, indent,
                      BuildXmlOpenTag("菜单", attributes, false));
    ...
    const auto findControlById =
        [&](std::int32_t childId) -> const FormInfo::ElementInfo * {
      if (const auto it = controlById.find(childId); it != controlById.end()) {
        return it->second;
      }

      for (const auto *item : controls) {
        if ((item->id & epl_system_id::kMaskNum) ==
            (childId & epl_system_id::kMaskNum)) {
          return item;
        }
      }
      return nullptr;
    ...
    std::function<void(const FormInfo::ElementInfo &, int)> renderControl =
        [&](const FormInfo::ElementInfo &item, int indent) {
          ...
          const bool isTabControl =
              resolver.IsTabControlDataType(item.dataType);
          const bool hasChildren = !item.children.empty();
          const bool hasEventBindings = !item.events.empty();
          if (!hasChildren && !hasEventBindings) {
            AppendXmlLine(formXml, indent,
                          BuildXmlOpenTag(tagName, attributes, true));
            return;
          }

          AppendXmlLine(formXml, indent,
                        BuildXmlOpenTag(tagName, attributes, false));
          AppendFormControlEventXmlLines(formXml, item, tagName, indent + 1,
                                         resolver);
          if (isTabControl) {
            ...
            for (const std::int32_t childId : item.children) {
              ...
              if (const auto *child = findControlById(childId);
                  child != nullptr) {
...
void BuildFormPage(const ModuleSections &sections, Document &outDocument) {
  ...
  for (const auto &item : sections.resources.forms) {
    AppendLine(page,
               BuildDefinitionLine("窗口", {TrimAsciiCopy(item.name),
                                            TrimAsciiCopy(item.comment)}));
  }
  outDocument.pages.push_back(std::move(page));
}

bool BuildDocumentFromSections(const ModuleSections &sections,
                               const std::string &sourcePath,
  ...
  document.projectName =
      sections.hasUserInfo &&
              !TrimAsciiCopy(sections.userInfo.programName).empty()
          ? TrimAsciiCopy(sections.userInfo.programName)
          : (sourcePath.empty() ? std::string("memory_serialize_project")
                                : Utf8PathToPath(sourcePath).stem().string());
  ...
  const AnonymousTypeHints anonymousTypeHints =
      BuildAnonymousTypeHints(sections, sourcePath);
...
std::string BuildProgramPageBaseClassName(const CodePageInfo &pageInfo,
                                          SymbolResolver &resolver) {
  std::string baseClassName;
  if (pageInfo.baseClass != 0 && pageInfo.baseClass != -1) {
    baseClassName = TrimAsciiCopy(resolver.ResolveType(pageInfo.baseClass));
  }
  if (baseClassName == "窗口" || baseClassName == "对象" ||
      baseClassName == "<对象>") {
    baseClassName.clear();
  }
...
std::string BuildPublicHeaderText(
    const ModuleSections &sections, const std::string &sourcePath,
    const std::vector<EComDependencyRecord> &dependencyRecords,
    const AnonymousTypeHints &anonymousTypeHints) {
  ...
  const std::string projectName =
      sections.hasUserInfo &&
          ...
          : (sourcePath.empty() ? std::string("memory_serialize_project")
                                : Utf8PathToPath(sourcePath).stem().string());
  const std::string author = sections.hasUserInfo
                                 ? TrimAsciiCopy(sections.userInfo.author)
                                 : std::string();
  ...
  const auto appendPublicMethod = [&lines, &resolver](
  ...
  for (const auto &item : sections.program.globals) {
    if ((item.attr & 0x0100) == 0 || IsGlobalHidden(item) ||
        IsDependencyDefinedId(dependencyRecords, item.marker)) {
      continue;
    }
    appendBlankLine();
  ...
  for (const auto &item : sections.program.dataTypes) {
    if (IsTxt2EPlaceholderStruct(item) || !IsStructPublic(item) ||
        IsStructHidden(item) ||
        IsDependencyDefinedId(dependencyRecords, item.header.dwId)) {
      continue;
    }
    ...
    if (typeName.empty()) {
      if (const auto it =
              anonymousTypeHints.localTypeAliases.find(item.header.dwId);
          it != anonymousTypeHints.localTypeAliases.end()) {
        typeName = it->second;
      } else {
        typeName = TrimAsciiCopy(resolver.ResolveUserName(item.header.dwId));
      }
    ...
    for (const auto &member : item.members) {
      lines.push_back(BuildStructMemberLine(
          member,
          ResolveMemberTypeWithHints(member, typeName, resolver,
                                     anonymousTypeHints),
          resolver));
    }
  }
  ...
  for (const auto &item : sections.program.dlls) {
    if ((item.attr & 0x2) == 0 || IsDllHidden(item) ||
        IsDependencyDefinedId(dependencyRecords, item.header.dwId)) {
      continue;
    }
    appendBlankLine();
    ...
    for (const auto &param : item.params) {
      lines.push_back(BuildDllParameterLine(
          param,
          ResolveDllParamTypeWithHints(param, dllName, resolver,
                                       anonymousTypeHints),
          resolver));
    }
  }
  ...
  for (const auto &item : sections.resources.constants) {
    if (item.pageType != kConstPageValue || item.valueText == "<图片>" ||
        item.valueText == "<声音>" || (item.attr & 0x2) == 0 ||
...
std::vector<std::string>
SplitTopLevelCommaFieldsForSnapshot(const std::string &text) {
...
bool ParseDefinitionFieldsForSnapshot(const std::string &line,
...
std::string GetSnapshotFieldOrEmpty(const std::vector<std::string> &fields,
                                    const size_t index) {
...
std::string ComputeGlobalSnapshotDigest(const VariableInfo &info,
                                        SymbolResolver &resolver) {
...
std::string ComputeDllParamSnapshotDigest(const VariableInfo &info,
                                          const std::string &resolvedType,
                                          SymbolResolver &resolver) {
  SnapshotVariableDef snapshot;
  snapshot.name = TrimAsciiCopy(resolver.ResolveUserName(info.marker));
...
std::string ComputeStructMemberSnapshotDigest(const VariableInfo &info,
                                              const std::string &resolvedType,
                                              SymbolResolver &resolver) {
  SnapshotVariableDef snapshot;
  snapshot.name = TrimAsciiCopy(resolver.ResolveUserName(info.marker));
...
std::string ComputeStructSnapshotDigest(const DataTypeInfo &item,
                                        SymbolResolver &resolver,
                                        const AnonymousTypeHints &hints) {
  std::ostringstream stream;
  std::string typeName = TrimAsciiCopy(item.name);
...
std::string BuildConstantSnapshotValueText(const ConstantInfo &item) {
  ...
  if (StartsWithText(valueText, kTextLiteralLeftQuote) &&
      EndsWithText(valueText, kTextLiteralRightQuote)) {
    valueText =
        BuildDumpTextLiteral(StripWrappedText(valueText, kTextLiteralLeftQuote,
                                              kTextLiteralRightQuote),
                             item.longText);
  } else if (!valueText.empty() &&
             valueText.find_first_of("eE") != std::string::npos &&
...
std::string ComputeResourceSnapshotDigest(const ConstantInfo &item,
                                          SymbolResolver &resolver) {
  const std::string dataDigest = ComputeTextDigest(
      std::string(reinterpret_cast<const char *>(item.rawData.data()),
                  item.rawData.size()));
  std::ostringstream stream;
...
std::string ComputeDllSnapshotDigest(const DllInfo &item,
                                     SymbolResolver &resolver,
                                     const AnonymousTypeHints &hints) {
  std::ostringstream stream;
...
bool TryBuildProgramPageSnapshot(const Page &page,
                                 SnapshotClassDef &outSnapshot) {
  outSnapshot = {};

  size_t index = 0;
  while (index < page.lines.size() &&
         TrimAsciiCopy(page.lines[index]) != ".版本 2") {
    ++index;
  }
  ...
  std::vector<std::string> fields;
  if (index >= page.lines.size() ||
      !ParseDefinitionFieldsForSnapshot(TrimAsciiCopy(page.lines[index]),
                                        "程序集", fields)) {
    return false;
  }
...
std::string BuildItemKey(const std::string &prefix, const std::string &rawName,
...
std::string FindCodePageNameById(const ProgramSection &program,
                                 const std::int32_t id) {
...
std::string BuildRelativePath(
    ...
    const std::int32_t folderKey, const std::string &baseDir,
    const std::string &fileName) {
  std::vector<std::string> parts;
...
bool BuildBundleFromSections(const ModuleSections &sections,
                             const std::string &sourcePath,
  ...
  bundle.publicHeaderText = BuildPublicHeaderText(
      sections, sourcePath, dependencyRecords, anonymousTypeHints);
  SymbolResolver resolver(sections.program, sections.resources, sourcePath,
                          &sections.losable.removedDefinedItems);
  ...
  for (const auto &item : sections.program.globals) {
    ...
    itemKeys.insert_or_assign(
        item.marker,
        BuildItemKey("global", resolver.ResolveUserName(item.marker),
                     keyCounters));
  }
  for (const auto &item : sections.program.dataTypes) {
    if (IsTxt2EPlaceholderStruct(item) || IsStructHidden(item) ||
    ...
    itemKeys.insert_or_assign(
        item.header.dwId,
        BuildItemKey("struct", resolver.ResolveUserName(item.header.dwId),
                     keyCounters));
  }
  for (const auto &item : sections.program.dlls) {
    if (IsDllHidden(item) ||
    ...
    itemKeys.insert_or_assign(
        item.header.dwId,
        BuildItemKey("dll", resolver.ResolveUserName(item.header.dwId),
                     keyCounters));
  }
  for (const auto &item : sections.resources.constants) {
    if (IsConstantHidden(item) ||
        IsDependencyDefinedId(dependencyRecords, item.marker)) {
      continue;
    }
    const std::string prefix =
        item.pageType == kConstPageImage
            ? "image"
            : (item.pageType == kConstPageSound ? "sound" : "constant");
    itemKeys.insert_or_assign(
        item.marker, BuildItemKey(prefix, resolver.ResolveUserName(item.marker),
                                  keyCounters));
  }
  for (const auto &item : sections.resources.forms) {
    itemKeys.insert_or_assign(
        item.header.dwId,
        BuildItemKey("form", resolver.ResolveUserName(item.header.dwId),
                     keyCounters));
  }
    ...
    if (page.typeName == "程序集") {
      while (programPageIndex < sections.program.codePages.size()) {
        ...
        const auto functions =
            CollectPageFunctions(sections.program, candidate);
        if (ShouldKeepPage(sections, dependencyRecords, candidate, functions,
                           true)) {
          break;
        }
        ++programPageIndex;
      ...
      file.relativePath = MakeUniqueRelativePath(
          BuildRelativePath(folderByKey,
                            itemFolderKey.contains(pageInfo.header.dwId)
                                ? itemFolderKey[pageInfo.header.dwId]
                                : 0,
                            "src", file.logicalName + ".txt"),
          usedRelativePaths);
      file.content = JoinPageLines(page.lines);
      bundle.sourceFiles.push_back(std::move(file));

      SnapshotClassDef pageSnapshot;
      const bool hasPageSnapshot =
          TryBuildProgramPageSnapshot(page, pageSnapshot);
      BundleNativeSourceFileSnapshot snapshot;
      snapshot.contentDigest =
          ComputeTextDigest(bundle.sourceFiles.back().content);
      if (hasPageSnapshot) {
        snapshot.classShapeDigest =
            ComputeSnapshotClassShapeDigest(pageSnapshot);
      }
      ...
      for (const auto *functionInfo : functions) {
        ...
        if (hasPageSnapshot &&
            snapshot.methods.size() < pageSnapshot.methods.size()) {
          const auto &methodTextSnapshot =
              pageSnapshot.methods[snapshot.methods.size()];
          methodSnapshot.name = methodTextSnapshot.name;
          methodSnapshot.textDigest =
              ComputeSnapshotMethodDigest(methodTextSnapshot);
        }
      ...
      for (const auto &item : sections.program.dataTypes) {
        ...
        snapshot.name =
            TrimAsciiCopy(resolver.ResolveUserName(item.header.dwId));
        snapshot.textDigest =
            ComputeStructSnapshotDigest(item, resolver, anonymousTypeHints);
      ...
      for (const auto &item : sections.program.dlls) {
        ...
        snapshot.name =
            TrimAsciiCopy(resolver.ResolveUserName(item.header.dwId));
        snapshot.textDigest =
            ComputeDllSnapshotDigest(item, resolver, anonymousTypeHints);
        snapshot.id = item.header.dwId;
        snapshot.memoryAddress = item.header.dwUnk;
  ...
  for (const auto &formXml : document.formXmls) {
    ...
    if (file.logicalName.empty()) {
      file.logicalName =
          TrimAsciiCopy(resolver.ResolveUserName(form.header.dwId));
    }
    file.relativePath = MakeUniqueRelativePath(
        BuildRelativePath(folderByKey,
                          itemFolderKey.contains(form.header.dwId)
                              ? itemFolderKey[form.header.dwId]
                              : 0,
                          "src", file.logicalName + ".xml"),
        usedRelativePaths);
    ...
    if (classKeyIt != itemKeys.end()) {
      ...
      binding.formName =
          TrimAsciiCopy(resolver.ResolveUserName(form.header.dwId));
      binding.className =
          TrimAsciiCopy(resolver.ResolveUserName(form.unknown2));
  ...
  for (const auto &item : sections.resources.constants) {
    ...
    resource.kind = item.pageType == kConstPageImage
                        ? BundleResourceKind::Image
                        : BundleResourceKind::Sound;
...
public:
  void WriteBool(const bool value) { WriteU8(value ? 1 : 0); }

  void WriteI32(const std::int32_t value) { WriteRaw(&value, sizeof(value)); }

  void WriteU64(const std::uint64_t value) { WriteRaw(&value, sizeof(value)); }

  void WriteU8(const std::uint8_t value) { WriteRaw(&value, sizeof(value)); }
...
bool CaptureNativeSectionSnapshots(
    ...
    std::vector<NativeSectionSnapshot> &outSnapshots, std::string *outError) {
  return CaptureNativeSectionSnapshotsInternal(inputBytes, outSnapshots,
                                               outError);
}
...
bool ExtractNativeDependencySymbols(
  ...
  const auto findRecordIndexById = [&](const std::int32_t id) -> size_t {
    ...
    for (size_t recordIndex = 0; recordIndex < dependencyRecords.size();
         ++recordIndex) {
      for (const auto &range : dependencyRecords[recordIndex].definedIds) {
        ...
        if (rangeType != idType && !(rangeType == epl_system_id::kTypeClass &&
                                     isClassLikeType(idType))) {
          continue;
        }
  ...
  SymbolResolver resolver(sections.program, sections.resources, std::string(),
                          &sections.losable.removedDefinedItems);
  std::unordered_map<std::int32_t, std::string> classNamesById;
  for (const auto &page : sections.program.codePages) {
    classNamesById.insert_or_assign(
        page.header.dwId,
        TrimAsciiCopy(resolver.ResolveUserName(page.header.dwId)));
  ...
  for (const auto &dataType : sections.program.dataTypes) {
    ...
    structSymbol.name =
        TrimAsciiCopy(resolver.ResolveUserName(dataType.header.dwId));
  ...
  for (const auto &function : sections.program.functions) {
    ...
    if (const auto ownerIt = classNamesById.find(function.ownerClass);
        ownerIt != classNamesById.end()) {
      methodSymbol.ownerClassName = ownerIt->second;
    }
    methodSymbol.name =
        TrimAsciiCopy(resolver.ResolveUserName(function.header.dwId));
  ...
  for (const auto &constant : sections.resources.constants) {
    ...
    const std::string name =
        TrimAsciiCopy(resolver.ResolveUserName(constant.marker));
    if (name.empty()) {
      continue;
    }
  ...
  for (const auto &removedItem : sections.losable.removedDefinedItems) {
    ...
    if (idType == epl_system_id::kTypeClass ||
      ...
      const bool exists =
          std::any_of(outRecords[recordIndex].classes.begin(),
      ...
      const bool exists =
          std::any_of(outRecords[recordIndex].structs.begin(),
                      outRecords[recordIndex].structs.end(),
                      [&](const NativeDependencyStructSymbol &item) {
                        return item.id == removedItem.id;
                      });
      if (!exists) {
        NativeDependencyStructSymbol structSymbol;
      ...
      const bool exists =
          std::any_of(outRecords[recordIndex].methods.begin(),
                      outRecords[recordIndex].methods.end(),
                      [&](const NativeDependencyMethodSymbol &item) {
                        return item.id == removedItem.id;
                      });
      if (!exists) {
        NativeDependencyMethodSymbol methodSymbol;
      ...
      const bool exists =
          std::any_of(outRecords[recordIndex].constants.begin(),
                      outRecords[recordIndex].constants.end(),
                      [&](const NativeDependencyConstantSymbol &item) {
                        return item.id == removedItem.id;
                      });
      if (!exists) {
        NativeDependencyConstantSymbol constantSymbol;
...
bool Generator::GenerateDocument(const std::string &inputPath,
                                 Document &outDocument, std::string *outError,
                                 const ReadOptions &readOptions) const {
  return GenerateDocumentInternal(inputPath, {}, readOptions, outDocument,
                                  outError);
}

bool Generator::GenerateBundle(const std::string &inputPath,
                               ProjectBundle &outBundle, std::string *outError,
                               const ReadOptions &readOptions) const {
  if (outError != nullptr) {
    outError->clear();
  }
  ...
  std::vector<std::uint8_t> inputBytes;
  if (!ReadModuleBytesWithReadOptions(inputPath, readOptions, inputBytes,
                                      outError)) {
    return false;
  }
...
bool ValidateNativeMethodBodyBytes(
    const std::vector<std::uint8_t> &expressionData, std::string *outError) {
  if (outError != nullptr) {
    outError->clear();
  }
...
bool TryMeasureNativeStatementEndOffset(
    const std::vector<std::uint8_t> &expressionData, size_t beginOffset,
    size_t &outEndOffset, std::string *outError) {
  ...
  bool ignoredMask = false;
  std::string parseError;
  if (!ParseCallExpressionWithoutType(reader, callExpr, &ignoredCode,
                                      &ignoredComment, &ignoredMask,
                                      &parseError)) {
    if (outError != nullptr) {
      *outError =
          parseError.empty() ? "statement_call_parse_failed" : parseError;
    }
    return false;
  }
...
bool Generator::GenerateDocumentFromBytes(
    const std::vector<std::uint8_t> &inputBytes, const std::string &sourcePath,
    Document &outDocument, std::string *outError) const {
  TraceLine("GenerateDocumentFromBytes begin source=" + sourcePath +
  ...
  TraceLine(
      "GenerateDocumentFromBytes parsed code_pages=" +
      std::to_string(sections.program.codePages.size()) +
  ...
  TraceLine("GenerateDocumentFromBytes pages_done count=" +
            std::to_string(outDocument.pages.size()));
...
bool Generator::GenerateBundleFromBytes(
    const std::vector<std::uint8_t> &inputBytes, const std::string &sourcePath,
    ProjectBundle &outBundle, std::string *outError) const {
  if (outError != nullptr) {
    outError->clear();
  }
...
bool Generator::GenerateDocumentInternal(const std::string &inputPath,
                                         const GenerateOptions &options,
                                         const ReadOptions &readOptions,
                                         Document &outDocument,
  ...
  TraceLine(
      "GenerateDocumentInternal parsed code_pages=" +
      std::to_string(sections.program.codePages.size()) +
      " functions=" + std::to_string(sections.program.functions.size()) +
      " globals=" + std::to_string(sections.program.globals.size()) +
      " data_types=" + std::to_string(sections.program.dataTypes.size()) +
  ...
  TraceLine("GenerateDocumentInternal pages_done count=" +
            std::to_string(outDocument.pages.size()));
  TraceLine("GenerateDocumentInternal end");
  return true;
}

bool Generator::GenerateText(const Document &document, std::string &outText,
                             std::string *outError) const {
  if (outError != nullptr) {
    outError->clear();
  }
  ...
  if (!document.dependencies.empty()) {
    ...
    for (const auto &item : document.dependencies) {
      if (item.kind == DependencyKind::ELib) {
        stream << "ELib name=" << item.name << " file=" << item.fileName
               << " guid=" << item.guid << " version=" << item.versionText
               << "\r\n";
      } else {
        stream << "ECom name=" << item.name << " path=" << item.path
  ...
  for (size_t i = 0; i < document.pages.size(); ++i) {
    ...
    stream << "================================================================"
              "================\r\n";
    stream << "[" << (i + 1) << "/" << document.pages.size()
           << "] type=" << page.typeName << " name=" << page.name << "\r\n";
    stream << "----------------------------------------------------------------"
              "----------------\r\n";
...
bool Generator::GenerateToFile(const std::string &inputPath,
                               const std::string &outputPath,
                               std::string *outSummary, std::string *outError,
  ...
  if (outSummary != nullptr) {
    *outSummary = "pages=" + std::to_string(document.pages.size()) +
                  ", output=" + outputPath;
  }
  return true;
}
